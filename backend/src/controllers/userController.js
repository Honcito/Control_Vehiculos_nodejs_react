...
const db = new sqlite3.Database(process.env.SQLITE_PATH || 'C:/database/control_vehiculos.db');
...
// Agrega aquí las demás funciones si las usas en las rutas
...
export const createUser = async (req, res) => { 
    ...
    try {
        ...
        db.run(sql, [nombre, hash, rol, telefono], function (err) {
          ...
          res
            .status(201)
            .json({ message: "Usuario creado con éxito", id: this.lasID });
        });
    } catch (error) {
...
export const updateUser = async (req, res) => {
  ...
  db.get("SELECT * FROM usuarios WHERE id_usuario = ?", [id],
    async (err, row) => {
      if (err) return res.status(500).json({ error: err.message });
